...
from segcraft.config.loader import load_and_validate_config
...
<head>
  ...
  <style>
    ...
    p { color: #5b6270; line-height: 1.5; }
    form, .panel { background: #fff; border: 1px solid #dde1e7; border-radius: 8px; padding: 18px; box-shadow: 0 1px 2px rgba(0,0,0,.04); }
    label { display: grid; gap: 6px; color: #333945; font-size: 14px; font-weight: 600; }
    input, select { border: 1px solid #cbd1dc; border-radius: 6px; padding: 10px 11px; font: inherit; min-width: 0; }
    .grid { display: grid; gap: 14px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
    .full { grid-column: 1 / -1; }
    button { border: 0; border-radius: 6px; background: #1769e0; color: white; padding: 11px 14px; font: inherit; font-weight: 700; cursor: pointer; }
    button:disabled { background: #94a3b8; cursor: wait; }
    .bar { height: 12px; background: #e5e9f0; border-radius: 999px; overflow: hidden; }
    .fill { width: 0%; height: 100%; background: #16a085; transition: width .25s ease; }
    .row { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
    .downloads { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
    .downloads a { color: #1769e0; border: 1px solid #cbd8f4; border-radius: 6px; padding: 8px 10px; text-decoration: none; font-weight: 600; }
...
<body>
  <main>
    <h1>SegCraft</h1>
    <p>Run a video segmentation job, watch progress, and download the generated outputs.</p>
    <form id="job-form" class="grid">
      <label class="full">Upload video <input name="video_file" type="file" accept="video/*"></label>
      <label class="full">YouTube URL <input name="youtube_url" type="url" placeholder="https://www.youtube.com/watch?v=..."></label>
      ...
      <label>Preset path <input name="preset_path" type="text" placeholder="configs/presets/cityscapes_video.yaml"></label>
